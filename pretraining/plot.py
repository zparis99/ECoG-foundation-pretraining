...
def get_seen_signal(model, original_signal, mask):
    """
    Creates an array with only the seen (unmasked) signal, filling masked parts with NaN.

    Args:
        model (MaskedAutoencoderViT): The instantiated model.
        original_signal (torch.Tensor): The original input signal (N, C, T, H, W).
        mask (torch.Tensor): The binary mask generated by forward_encoder (N, L),
                              where 0 is keep and 1 is remove.

    Returns:
        np.ndarray: The signal with masked patches set to NaN, in the original (N, C, T, H, W) shape.
    """
    # Detach and clone original signal to avoid modifying it
    original_signal_tensor = original_signal.clone().detach()

    # Patchify the original signal
    original_patches = model.patchify(original_signal_tensor)  # (N, L, patch_dim)

    seen_patches = torch.full_like(original_patches, float("nan"))

    N, L, patch_dim = original_patches.shape
    kept_mask = (mask == 0).unsqueeze(-1).expand_as(original_patches)

    seen_patches_filled = torch.where(kept_mask, original_patches, seen_patches)

    # Unpatchify to get the signal in the original shape, with masked areas as NaN
    seen_signal = model.unpatchify(seen_patches_filled)
    return seen_signal.detach().cpu().numpy()
